...
@Component({
  selector: 'app-telimedicine-report',
  templateUrl: './telimedicine-report.component.html',
  styleUrls: ['./telimedicine-report.component.css']
})
export class TelimedicineReportComponent implements OnInit, AfterViewInit {
  ...
  patientCountAccordingToDateAndBranch(){
    ...
    if (this.patientSearchFormByDate.value.startDate && this.patientSearchFormByDate.value.endDate){
      ...
      this.reportService.getTelimedicineListAccordingToDateAndBranch(startDate, endDate).subscribe(response => {
